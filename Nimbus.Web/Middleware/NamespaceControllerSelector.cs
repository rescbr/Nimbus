...
namespace Nimbus.Web.Middleware
{
    public class NamespaceHttpControllerSelector : IHttpControllerSelector
    {
        ...
        private Dictionary<string, HttpControllerDescriptor> InitializeControllerDictionary()
        {
            ...
            {
                List<string> segments = t.Namespace.Split(Type.Delimiter).ToList();
                segments.RemoveAll(s => s.Contains("Controller"));

                // For the dictionary key, strip "Controller" from the end of the type name.
                // This matches the behavior of DefaultHttpControllerSelector.
                var controllerName = t.Name.Remove(t.Name.Length - DefaultHttpControllerSelector.ControllerSuffix.Length);

                var key = String.Format(CultureInfo.InvariantCulture, "{0}.{1}", segments[segments.Count - 1], controllerName);
